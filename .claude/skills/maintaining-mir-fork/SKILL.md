...
- **One topic branch per fix, branched off upstream `master`**, each independently PR-able with its own regression test: `fix-laddr-out-flag`, `fix-gvn-load-ext`, `fix-jump-opt-lref-labels`, `fix-aarch64-ld-stack-align`, `support-musl-std-libs`, `pr-420` (cherry-picked foreign PR).
...
| aarch64 musl (Alpine) | Those 3 do NOT reproduce. Instead: `c-tests/new/jcall.c` segfaults after main returns in gen modes only (exotic `__builtin_jcall`/jret + global reg var; passes interp and glibc; we never emit JCALL). `c2mir-bb-bootstrap-test` OOMs below ~4GB RAM: deterministic 1.78GB anon-RSS / 4.47GB total-VM even at `-p1` with idle swap — c2m-on-musl(mallocng) peaks ~2.5× glibc (cf. upstream #411); not codegen, not our production surface |
...
Open PRs: **#430** (LADDR out-flag + simplified-RA jmpi), **#432** (GVN store-forwarding loses sign/zero extension, fixes #423), **#433** (jump_opt frees LADDR/lref labels → UAF, fixes #424), **#434** (aarch64 `% 16` where round-up meant — mir-aarch64.c va_arg `__stack` and ff_call sp_offset; fixes #431), **#435** (musl std-lib tables in the test drivers, fixes #307). Cherry-picked: #420 (error-path null deref). Watchlist: #383 (`MIR_get_global_item` undefined — cherry-pick when needed), #426 (lref breaks .bmir round-trip — blocks caching computed-goto modules), #429 (did NOT reproduce), #411 (c2m memory).
...
Ephemeral EC2 instances, `ssh -i ~/.ssh/General.pem` — IPs change every start, ask the user for current ones. Ubuntu aarch64: user `ubuntu`. Alpine aarch64: user `alpine`; **login shell is fish** — wrap everything in `sh -c '...'` or `ssh ... 'sh -s' <<'EOF'`; privilege escalation is `doas`, not sudo. When polling a remote suite, `pgrep -f "make test"` matches its own ssh wrapper — use `pgrep -a make`. Full `make test` needs ≥4GB RAM on musl (the bb-bootstrap ceiling above).
